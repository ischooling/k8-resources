			...
			if (data['status'] == '0' || data['status'] == '2') {
				...
				if(pageType=='internal'){
					...
					$.each(data['pageListByVendorId'], function(k, v) {
						html+='<li id="sub-li-id-'+indexPosition+'" class=""><a href="javascript:void(\'0\');" onclick="return renderIframe(\''+v.encryptedPageId+'\',\''+SCHOOL_UUID+'\');">'+v.pageName+'</a></li>';
						indexPosition++;
					});
					$('#'+elementId).html(html);
...
function showRaisedTicketByUser(src){
	...
	callForDashboardData('formIdIfAny','ticket-raised-listing');
	$(src).addClass('active');
}
				...
				$.each(data['ticketListDetailDTO'], function(k, v) {
					html+='<tr>'+
						'<td style="text-align:center;">'+indexPosition+'</td>'+
							...
							'<a onclick="callForDashboardData(\'formIdIfAny\',\'ticket-raised-user-info?ticketId='+v.ticketId+'\', \'tickerRaiseInfoDiv\')" href="javascript:void(0);"  style="margin-left: 45%;">'+
									'<i class="fa fa-eye" style="margin-right: 5px; color: #067e06; font-size: 18px;"></i>'+
							'</a>'+
						'</td>'+
					'</tr>';
					indexPosition++;
